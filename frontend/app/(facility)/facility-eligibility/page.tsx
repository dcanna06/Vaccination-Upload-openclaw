...
import { RESIDENTS, getFacilityResidents, getAge } from '@/lib/mock/portal-data';
import { VACCINE_NAMES } from '@/types/portals';

const FACILITY_ID = 2;
const VACCINE_CODES = ['flu', 'covid', 'shingrix', 'pneumo', 'dtpa'] as const;

export default function FacilityEligibilityPage() {
  ...
  const toggleVaccine = (code: string) => {
    setSelectedVaccines(prev => prev.includes(code) ? prev.filter(c => c !== code) : [...prev, code]);
  };

  const toggleAll = () => {
    setSelectedVaccines(prev => prev.length === VACCINE_CODES.length ? [] : [...VACCINE_CODES]);
  };

  // Residents with at least one selected vaccine that is due/overdue
  const eligibleResidents = useMemo(() => {
    return residents
      .filter(r => selectedVaccines.some(v => {
  ...
  }, [residents, selectedVaccines, sortAsc]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <StatTile label="Overdue" value={overdueCount} icon={AlertTriangle} color="#b91c1c" bgColor="#fef2f2" subtitle="Need attention now" />
        <StatTile label="Coming Due" value={comingDueCount} icon={Clock} color="#c2410c" bgColor="#fff7ed" subtitle="In the next 60 days" />
        <StatTile label="Total Eligible" value={eligibleResidents.length} icon={Syringe} color="#3B6CE7" bgColor="#eff6ff" subtitle={`Across ${selectedVaccines.length} vaccines`} />
      </div>
          ...
          <thead>
            <tr className="border-b bg-gray-50 text-left text-xs uppercase text-gray-500">
              ...
              {selectedVaccines.map(v => (
                <th key={v} className="px-4 py-3 text-center">{VACCINE_NAMES[v]}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {eligibleResidents.map((r, idx) => (
              <tr key={r.id} className={`border-b last:border-0 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                <td className="px-4 py-3">
                  <p className="font-medium text-gray-900">{r.firstName} {r.lastName}</p>
                  {r.wing && <p className="text-xs text-gray-400">{r.wing}</p>}
                </td>
                <td className="px-4 py-3 text-gray-700">{r.room}</td>
                <td className="px-4 py-3 text-gray-700">{getAge(r.dateOfBirth)}</td>
                {selectedVaccines.map(v => {
                  const e = r.eligibility[v];
                  if (!e) return <td key={v} className="px-4 py-3 text-center text-gray-400">—</td>;
                  if (e.isOverdue) {
                    return (
                  ...
                  if (e.isDue) {
                    return (
                      <td key={v} className="px-4 py-3 text-center">
                        <Pill color="orange">
                          {e.dueDate ? new Date(e.dueDate).toLocaleDateString('en-AU', { month: 'short', day: 'numeric' }) : 'Due'}
                        </Pill>
                      </td>
                    );
                  }
            ...
            {eligibleResidents.length === 0 && (
              <tr><td colSpan={3 + selectedVaccines.length} className="px-4 py-8 text-center text-gray-500">No residents match the selected criteria.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function getEarliestDueDate(resident: { eligibility: Record<string, { isDue: boolean; isOverdue: boolean; dueDate?: string }> }, vaccines: string[]): string {
